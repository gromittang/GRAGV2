...
> **设计决策详见**: [ADR-011: 行业自适应分块策略](../adr/adr-011-chunk-strategy-optimization.md)
...
| 行业 (industry) | chunk_size | chunk_overlap | top_k | BM25 | 设计理由 |
|-----------------|-----------|---------------|-------|------|---------|
| general (默认)  | 500       | 100           | 5     | ✅   | 通用平衡 |
| wms             | 800       | 150           | 8     | ✅   | 操作流程长，大窗口保留完整 SOP 上下文 |
| medical         | 300       | 50            | 10    | ✅   | 文档碎片化，小块精确检索 |
| legal           | 1000      | 200           | 5     | ❌   | 法律条文长，纯向量检索更准确 |
| finance         | 600       | 100           | 6     | ✅   | 介于通用和 WMS 之间 |

配置获取逻辑: 根据知识库关联的行业设置读取对应参数，未匹配时回退到 `general`。

**设计原则**: chunk_overlap ≈ chunk_size × 15~20%，确保语义边界不丢失。检索参数（top_k, BM25 开关）与分块参数联动：小块多召回，大块少召回。
