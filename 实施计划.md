...
访问 http://localhost:8811/health 验证服务启动
...
# 测试配置获取
curl http://localhost:8811/config

# 测试对话（无文档）
curl -X POST http://localhost:8811/api/v1/chat/ \
...
```bash
# 上传测试文档
curl -X POST http://localhost:8811/api/v1/documents/upload \
  -F "file=@test.txt" \
  -F "knowledge_name=测试知识库"

# 查看文档列表
curl http://localhost:8811/api/v1/documents/list/1/10
```
...
docker run -p 8811:8811 --env-file .env wmsrag-v2:test
```
...
3. **Docker 构建**
   ...
   docker run -p 8811:8811 --env-file .env kb-system:latest
   ```
